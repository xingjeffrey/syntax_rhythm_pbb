...
def construct_transition_cube(note_df, label = 'lasso_labels'):
    '''
    Construct Transition Cube
    '''
    
    ## find a sequence
    seq_container = list(note_df[label].values)
    
    ## find unique symbols
    unique_states = np.unique(seq_container)
    
    ## set up transition cube
    transition_cube = np.zeros((len(unique_states), len(unique_states), len(unique_states)))
    
    ## populate transition cube with tallies
    for note_nb in range(len(seq_container) - 2):
        ##slot into cube as transition_cube[past2][past1][current] + 1 tally
        past2 = seq_container[note_nb]
        past1 = seq_container[note_nb + 1]
        current = seq_container[note_nb + 2]
        
        ## add tally 1 at proper location
        transition_cube[past2][past1][current] += 1
        
    ## convert tallies into probabilities
    p1index = 0
    p2index = 0

    ## given past1 and past2
    for past1 in transition_cube:
        for past2 in past1:
            ## probability of third symbol given past1 and past2
            if sum(past2) == 0:
                transition_cube[p1index][p2index] = transition_cube[p1index][p2index]
            else:
                transition_cube[p1index][p2index] = transition_cube[p1index][p2index]/sum(past2)
            p2index += 1
        p2index = 0
        p1index += 1
        
    return transition_cube
...
def syn_seq_gen(indv_df, label = 'hdbscan_labels', order = 'zero'):
    ...
    if order == 'second':
        first_order_model = construct_seq_matrix(indv_df, label = label, sort = False)
        second_order_model = construct_transition_cube(indv_df, label = label)
        
        ## generate first symbol
        past1 = random.choices(unique_states, p_states)[0]
        
        ## generate second symbol
        matrix_solution = first_order_model.transpose()[past1].values
        
        past2 = random.choices(unique_states, matrix_solution)[0]
        
        ## generate the rest with second-order model
        for stated in tqdm(range(len(indv_df_cc[label]))):
            ...
            except: ## if error, continue with a first-order
                matrix_solution = first_order_model.transpose()[past1].values
                current = random.choices(unique_states, matrix_solution)[0]
