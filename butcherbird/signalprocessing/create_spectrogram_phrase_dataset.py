...
def subset_syllables(
    json_dict, indv, unit="phrases", hparams=None, include_labels=True
):
    """ Grab phrases from wav data
    """
    if type(indv) == list:
        indv = indv[0]
    ...
    # get unit info
    phrase_strt = json_dict["indvs"][indv][unit]["phrase_strt"]
    # stop times vs phrase_end is a quick fix that should be fixed on the parsing side
    if "phrase_end" in json_dict["indvs"][indv][unit].keys():
        phrase_end = json_dict["indvs"][indv][unit]["phrase_end"]
    else:
        phrase_end = json_dict["indvs"][indv][unit]["stop_times"]
    if include_labels:
    ...
    syllables = [
        data[int(st * rate) : int(et * rate)] for st, et in zip(phrase_strt, phrase_end)
    ]
...
def get_element(
    ...
    st = element["phrase_strt"][element_number]
    et = element["phrase_end"][element_number]
...
def create_label_df(
    ...
    for indvi, indv in enumerate(json_dict["indvs"].keys()):
        ...
        for label in labels_to_retain:
            ...
            if len(indv_dict[label]) < len(indv_dict["phrase_strt"]):
                indv_dict[label] = np.repeat(
                    indv_dict[label], len(indv_dict["phrase_strt"])
                )
...
def get_row_audio(syllable_df, wav_loc, hparams):
    ...
    syllable_df["audio"] = [
        ...
        for st, et in zip(syllable_df.phrase_strt.values, syllable_df.phrase_end.values)
    ]
