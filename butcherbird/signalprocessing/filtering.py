...
def bbf_updated_audio(df):
    # for every note, bandpass the range of prominent signal

    bp_width = 1
    audio_temp = []
    df = df.iterrows()

    for index, note in df:
        # get window
        f_pair = [note['f_start'], note['f_end']]
        # sort them [low, high] + offset
        f_window = [np.amin(f_pair) - bp_width, np.amax(f_pair) + bp_width]

        # use bandpass filter and rewrite audio into audio_temp
        audio_temp.append(
    ...
    audio_updater = pd.DataFrame(data = {'audio':audio_temp})
    return audio_updater
