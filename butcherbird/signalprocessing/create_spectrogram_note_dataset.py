...
def subset_syllables(
    json_dict, indv, unit="notes", hparams=None, include_labels=True
):
    """ Grab notes from wav data
    """
    if type(indv) == list:
        indv = indv[0]
    ...
    # get unit info
    note_strt = json_dict["indvs"][indv][unit]["note_strt"]
    # stop times vs note_end is a quick fix that should be fixed on the parsing side
    if "note_end" in json_dict["indvs"][indv][unit].keys():
        note_end = json_dict["indvs"][indv][unit]["note_end"]
    else:
        note_end = json_dict["indvs"][indv][unit]["stop_times"]
    if include_labels:
    ...
    syllables = [
        data[int(st * rate) : int(et * rate)] for st, et in zip(note_strt, note_end)
    ]
...
def log_resize_spec(spec, scaling_factor=10):
    ...
    print(resize_shape)
    ## override and create lower bound
    if (resize_shape[0] == 0):
        resize_shape[0] = 1
...
def pad_spectrogram(spectrogram, pad_length, mode = 'center'):
    ...
    if mode == 'left':
        return np.pad(
            spectrogram, [(0, 0), (0, excess_needed)], "constant", constant_values=0
        )
    if mode == 'right':
        return np.pad(
            spectrogram, [(0, 0), (excess_needed, 0)], "constant", constant_values=0
        )
...
def get_element(
    ...
    st = element["note_strt"][element_number]
    et = element["note_end"][element_number]
...
def create_label_df(
    ...
    for indvi, indv in enumerate(json_dict["indvs"].keys()):
        ...
        # if selected unit is not in the dictionary, skip over
        '''
        if unit not in json_dict["indvs"][indv].keys():
            continue
        '''    
        # generate dict for each individual
        indv_dict = {}
        
        # pull strt and end for selected unit
        indv_dict["phrase_nb"] = json_dict["indvs"][indv]["units"]['phrases']["nb"]
        indv_dict["phrase_strt"] = json_dict["indvs"][indv]["units"]['phrases']["strt"]
        indv_dict["phrase_end"] = json_dict["indvs"][indv]["units"]['phrases']["end"]
        indv_dict["phrase_len"] = json_dict["indvs"][indv]["units"]['phrases']["len"]
        indv_dict["note_cnt"] = json_dict["indvs"][indv]["units"]['notes']["cnt"]
        indv_dict["note_nb"] = json_dict["indvs"][indv]["units"]['notes']["nb"]
        indv_dict["note_strt"] = json_dict["indvs"][indv]["units"]['notes']["strt"]
        indv_dict["note_end"] = json_dict["indvs"][indv]["units"]['notes']["end"]
        indv_dict["note_len"] = json_dict["indvs"][indv]["units"]['notes']["len"]

        # get data for individual
        for label in labels_to_retain:
            indv_dict[label] = json_dict["indvs"][indv][unit][label]
            
            # if length of the label dictionary is too short, repeat and fill to the size of strt
            if len(indv_dict[label]) < len(indv_dict["note_strt"]):
                indv_dict[label] = np.repeat(
                    indv_dict[label], len(indv_dict["note_strt"])
                )
...
def get_row_audio(syllable_df, wav_loc, hparams, mode = 'note'):
    ...
    syllable_df["audio"] = [
        ...
        for st, et in zip(syllable_df.note_strt.values, syllable_df.note_end.values)
    ]
